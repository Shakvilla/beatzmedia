 ...
 * via {@link TokenIssuer} — the same infra {@code LoginService} uses, but with a short, distinct
 * TTL ({@code beatz.jwt.impersonation-ttl-seconds}, independently tunable from the normal login
 * TTL) and a role set that deliberately EXCLUDES any admin role, even if the target happens to be
 * an admin member (security default: impersonation investigates regular users, it never grants
 * one admin another admin's privileges — see admin ADD WU-ADM-2 as-built notes). Does NOT append
 * an AuditEntry — the {@code admin} module owns the heavily-audited INV-10 record (actor, target,
...
public class IssueImpersonationTokenService implements IssueImpersonationToken {
  ...
  @Override
  @Transactional
  public ImpersonationTokenView issue(AccountId target) {
    ...
    String token = tokenIssuer.issue(account.getId(), scopes, ttl);
