...
export function Toggle({ checked, onChange, label, disabled }: { checked: boolean; onChange: (v: boolean) => void; label?: string; disabled?: boolean }) {
  return (
    <button
      ...
      className={cn('w-11 h-6 rounded-full p-0.5 transition-colors shrink-0 disabled:opacity-40 disabled:cursor-not-allowed', checked ? 'bg-beatz-green' : 'bg-gray-300 dark:bg-white/20')}
    >
